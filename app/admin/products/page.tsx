...
// ─── usePreline — re-init Preline after dynamic content renders ───────────────
      ...
      >
        {preview ? (
          <>
            ...
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); setPreview(null); onFileChange(null); }}
...
function CategorySelect({ categories, loading, value, onChange, onCategoryCreated }: {
  ...
  return (
    <div className="space-y-2">
      {/* Native select — straightforward, no Preline JS needed */}
      <select
        value={value}
        onChange={(e) => {
...
function AddModal({ onCreated }: { onCreated: (p: DisplayProduct) => void }) {
  ...
  return (
    <>
      ...
      <button
        type="button"
        aria-haspopup="dialog" aria-expanded="false" aria-controls={modalId}
        ...
        <div className="hs-overlay-open:opacity-100 hs-overlay-open:duration-300 opacity-0 transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
          <div className="pointer-events-auto flex flex-col bg-white border shadow-sm rounded-xl dark:bg-neutral-800 dark:border-neutral-700">
            ...
            <div className="p-4 overflow-y-auto max-h-[calc(100vh-12rem)]">
              ...
              <ProductFormFields
                form={form as ReturnType<typeof makeEmptyForm>}
                setForm={setForm}
                files={files} setFiles={setFiles}
                categories={categories} catLoading={catLoading} addCategory={addCategory}
              />
              {saveError && (
                <div className="mt-3 p-3 text-sm text-red-600 rounded-lg bg-red-50 dark:bg-red-900/20 dark:text-red-400">{saveError}</div>
              )}
            </div>
            ...
            <div className="flex justify-end items-center gap-x-2 py-3 px-4 border-t dark:border-neutral-700">
              <button
                type="button" data-hs-overlay={`#${modalId}`} onClick={reset}
...
function EditModal({ product, onSave, onClose }: { product: DisplayProduct; onSave: (p: DisplayProduct) => void; onClose: () => void }) {
  ...
  const [form, setForm] = useState<DisplayProduct & ReturnType<typeof makeEmptyForm>>({ ...product } as DisplayProduct & ReturnType<typeof makeEmptyForm>);
  const [files, setFiles] = useState<Record<string, File | null>>({});
  const [saving, setSaving] = useState(false);
  const [saveError, setSaveError] = useState<string | null>(null);
      ...
      <div className="hs-overlay-open:opacity-100 hs-overlay-open:duration-300 opacity-0 transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
        <div className="pointer-events-auto flex flex-col bg-white border shadow-sm rounded-xl dark:bg-neutral-800 dark:border-neutral-700">
          ...
          <div className="p-4 overflow-y-auto max-h-[calc(100vh-12rem)]">
            ...
            <ProductFormFields
              form={form} setForm={setForm as (f: ReturnType<typeof makeEmptyForm>) => void}
              files={files} setFiles={setFiles}
              categories={categories} catLoading={catLoading} addCategory={addCategory}
            />
            {saveError && (
...
export default function ProductsAdminPage() {
  ...
  return (
    <div className="space-y-4">
                ...
                <thead className="bg-gray-50 dark:bg-neutral-800">
                  <tr>
                    {["ID", "Image", "Name", "Brand", "Category", "Tags", "Price", "Stock", "Status", ""].map((h) => (
                      <th key={h} className="px-4 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-neutral-400">
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {products.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {product.heroImage ? (
                          <img
                            src={product.heroImage} alt={product.name}
                        ...
                        <div className="flex flex-wrap gap-1">
                          {product.tags.length > 0
                            ? product.tags.map((tag) => (
                                <span key={tag} className="inline-flex items-center rounded-md bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600 dark:bg-neutral-700 dark:text-neutral-300">
                                  {tag}
                                </span>
                              ))
      ...
      {/* Delete modal — mounted on demand */}
      {deleteTarget && (
        <DeleteModal
          key={deleteTarget.id}
