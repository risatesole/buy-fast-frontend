...
type SortField = 'name' | 'email' | 'created_at' | 'purchases';
type SortDirection = 'asc' | 'desc';
type InstitutionMember = '' | 'true' | 'false';

const SORT_FIELD_OPTIONS: { value: SortField; label: string }[] = [
  { value: 'created_at', label: 'Fecha de registro' },
  { value: 'name', label: 'Nombre' },
  { value: 'email', label: 'Email' },
  { value: 'purchases', label: 'Cantidad de compras' },
];

const INSTITUTION_MEMBER_OPTIONS: { value: InstitutionMember; label: string }[] = [
  { value: '', label: 'Todos' },
  { value: 'true', label: 'Miembro UASD' },
...
export default function CustomersReportPage() {
  ...
  const [institutionMember, setInstitutionMember] = useState<InstitutionMember>('');
  ...
  const buildDownloadUrl = useCallback(
    (reportFormat: 'pdf' | 'csv') => {
      ...
      if (institutionMember) params.set('institution_member', institutionMember);
      if (dateFrom) params.set('date_from', dateFrom);
      if (dateTo) params.set('date_to', dateTo);
      if (minPurchases) params.set('min_purchases', minPurchases);
      if (maxPurchases) params.set('max_purchases', maxPurchases);
      params.set('sort', sortDirection === 'desc' ? `-${sortField}` : sortField);
      params.set('report_format', reportFormat);
      return `/api/v1/admin/reports/customers?${params.toString()}`;
    },
    [
      search,
      institutionMember,
    ...
    <div className="flex flex-col h-full bg-[#f7f9fb]">
      <header className="flex items-center justify-between px-8 py-6 bg-white border-b border-[#e0e3e5]">
        <div>
          ...
          <p className="text-[13px] font-sans text-[#747781] mt-1">
            Genere reportes de clientes filtrados en formato PDF o CSV.
          </p>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar p-8">
        <div className="max-w-3xl bg-white rounded-lg border border-[#e0e3e5] p-6">
          <h2 className="text-sm font-semibold text-[#191c1e] mb-5">Clientes</h2>

          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
              <label className={labelClass} htmlFor="customers-search">
            ...
            <div>
              <label className={labelClass} htmlFor="customers-institution-member">
                Membresía
              </label>
              <select
                id="customers-institution-member"
                className={inputClass}
                value={institutionMember}
                onChange={e => setInstitutionMember(e.target.value as InstitutionMember)}
              >
                {INSTITUTION_MEMBER_OPTIONS.map(option => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={labelClass} htmlFor="customers-date-from">
                  Registrado desde
                </label>
                <input
              ...
              <div>
                <label className={labelClass} htmlFor="customers-date-to">
                  Hasta
                </label>
                <input
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={labelClass} htmlFor="customers-min-purchases">
                  Compras mín.
                </label>
                <input
                  id="customers-min-purchases"
                  type="number"
                  min={0}
                  className={inputClass}
                  value={minPurchases}
              ...
              <div>
                <label className={labelClass} htmlFor="customers-max-purchases">
                  Compras máx.
                </label>
                <input
                  id="customers-max-purchases"
                  type="number"
                  min={0}
                  className={inputClass}
                  value={maxPurchases}
            ...
            <div>
              <label className={labelClass} htmlFor="customers-sort-field">
                Ordenar por
              </label>
              <select
            ...
            <div>
              <label className={labelClass} htmlFor="customers-sort-direction">
                Dirección
              </label>
              <select
                id="customers-sort-direction"
                className={inputClass}
                value={sortDirection}
